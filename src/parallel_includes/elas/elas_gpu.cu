 ...
 */
__global__ void findMatch_GPU(int32_t *u_vals,
                              int32_t *v_vals,
                              int32_t size_total,
                              float *planes_a,
                              float *planes_b,
    ...
    uint32_t d_addr;
    if (subsampling)
        d_addr = getAddressOffsetImage_GPU(u / 2, v / 2, width / 2);
    else
 ...
 */
void ElasGPU::computeDisparity(std::vector<support_pt> p_support,
    ...
    if (!this->memInitDone)
        memInit(size_total, pixs_u, pixs_v, disp_num, grid_dims);
    ...
    // Launch the kernel
    findMatch_GPU<<<DimGrid, DimBlock>>>(d_u_vals, d_v_vals, size_total, d_planes_a, d_planes_b, d_planes_c, d_disparity_grid, d_grid_dims, d_I1,
                                         d_I2, d_P, plane_radius, width, height, d_valids, right_image, d_D, false);
