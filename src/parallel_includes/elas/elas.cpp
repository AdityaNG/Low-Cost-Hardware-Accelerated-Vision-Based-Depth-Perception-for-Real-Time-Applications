            ...
            if (d_can >= 0) {
                ...
                for (int32_t u_can_2 = u_can - param.incon_window_size; u_can_2 <= u_can + param.incon_window_size; u_can_2++) {
                    for (int32_t v_can_2 = v_can - param.incon_window_size; v_can_2 <= v_can + param.incon_window_size; v_can_2++) {
                        // Check we're inside candidate array (slightly smaller
                        // than image)
                        if (u_can_2 >= 0 && v_can_2 >= 0 && u_can_2 < D_can_width && v_can_2 < D_can_height) {
                            ...
                            // Check if the disparity is different above a given
                            // threshold (5 pixels) If it is considered fine,
                            // similar to other pixels around it, consider it a
                            // support
                ...
                // invalidate support point if number of supporting points is
                // too low
...
/*
 * Removes support points with the same disparity along horizontal and vertical
 * lines
 */
void Elas::removeRedundantSupportPoints(int16_t *D_can,
                                        int32_t D_can_width,
                                        int32_t D_can_height,
                                        int32_t redun_max_dist,
                                        int32_t redun_threshold,
                                        bool vertical) {
    ...
    for (int32_t u_can = 0; u_can < D_can_width; u_can++) {
        for (int32_t v_can = 0; v_can < D_can_height; v_can++) {
            ...
            if (d_can >= 0) {
                ...
                bool redundant = true;
                // Loop through horizontal directions (left,right) or vertical
                // direction (up,down)
                for (int32_t i = 0; i < 2; i++) {
                    ...
                    int16_t d_can_2;
                    bool support = false;
                    // Loops the spacial coordinate in the respective direction
                    // until the max distance for checking
                    for (int32_t j = 0; j < redun_max_dist; j++) {
                        ...
                        // If the neighboring point has a similar disparity
                        // below the threshhold (1)
                        // TODO: What if jump in disparity
                        if (d_can_2 >= 0 && abs(d_can - d_can_2) <= redun_threshold) {
                            support = true;  // We are have a neighboring thats
                                             // very similar
                            break;
                        }
                    }
...
inline int16_t Elas::computeMatchingDisparity(const int32_t &u, const int32_t &v, uint8_t *I1_desc, uint8_t *I2_desc, const bool &right_image) {
    ...
    if (u >= window_size + u_step && u <= width - window_size - 1 - u_step && v >= window_size + v_step && v <= height - window_size - 1 - v_step) {
        ...
        // If our candidate had a texture below a threshold (Aka not a line)
        // dont use it
        ...
        // Limits how far we can go out based on window and surrounding
        ...
        for (int16_t d = disp_min_valid; d <= disp_max_valid; d++) {
            ...
            // Sum all the intensity differences of all the surrounding
            // canditates between the first I1 and second I2 inages
            xmm6 = _mm_load_si128((__m128i *)(I2_block_addr + desc_offset_1));  // Get NE candidate from image 2
            //_mm_sad_epu8 sums the difference of the first 64 bytes and second
            // 64 bytes (returns 2 values)
            xmm6 = _mm_sad_epu8(xmm1,
                                xmm6);                                          // Subtract the two values from image 1 and image 2
            ...
            if (sum < min_1_E) {
                min_2_E = min_1_E;  // Absent in OpenMP version
                min_2_d = min_1_d;  // Absent in OpenMP version
                min_1_E = sum;
                min_1_d = d;
            } else if (sum < min_2_E) {
                min_2_E = sum;
        ...
        // check if best and second best match are available and if matching
        // ratio is sufficient Threshold says the second is within the 95th
...
vector<Elas::support_pt> Elas::computeSupportMatches(uint8_t *I1_desc, uint8_t *I2_desc) {
    ...
    for (int32_t u = 0; u < width; u += D_candidate_stepsize)
        D_can_width++;  // Determine number of candidates at the stepsize in the
                        // horizontal
    for (int32_t v = 0; v < height; v += D_candidate_stepsize)
        D_can_height++;  // Determine number of candidates at the stepsize in
                         // the vertical
...
#pragma omp parallel default(none) num_threads(2) private(u_can, v_can, u, d, v, d2) \
    shared(partial_p_support, lr_threshold, D_can, D_can_width, D_can_height, D_candidate_stepsize, I1_desc, I2_desc)
    {
        ...
        for (v_can = 1; v_can < D_can_height; v_can++) {
            ...
            for (u_can = 1; u_can < D_can_width; u_can++) {
                ...
                *(D_can + getAddressOffsetImage(u_can, v_can, D_can_width)) = -1;  // Find the current candidate location
                ...
                // find forwards
                d = computeMatchingDisparity(u, v, I1_desc, I2_desc,
                                             false);  // Left image
                // If we have found a disparity in the left, check from the
                // right to left disparity
                if (d >= 0) {
                    // find backwards
                    d2 = computeMatchingDisparity(u - d, v, I1_desc, I2_desc, true);
                    // Check our error between the 2 disparity and that its
                    // below 2 pixel difference
                    if (d2 >= 0 && abs(d - d2) <= lr_threshold)                           // TODO: Use average of the two? Why
                                                                                          // only the left?
        ...
        for (int32_t v_can = 1; v_can < D_can_height; v_can++)
            for (int32_t u_can = 1; u_can < D_can_width; u_can++)
                if (*(D_can + getAddressOffsetImage(u_can, v_can, D_can_width)) >= 0)
                    partial_p_support[tid].push_back(support_pt(u_can * D_candidate_stepsize, v_can * D_candidate_stepsize,
...
void Elas::computeDisparityPlanes(vector<support_pt> p_support, vector<triangle> &tri, int32_t right_image) {
    ...
    for (int32_t i = 0; i < tri.size(); i++) {
        ...
        // The triangle indecties correspond directly to the support point
        // indecies
        int32_t c1 = tri[i].c1;
        int32_t c2 = tri[i].c2;
        int32_t c3 = tri[i].c3;
...
inline void Elas::findMatch(int32_t &u,
    ...
    if (!right_image) {
        for (int32_t i = 0; i < num_grid; i++) {
            d_curr = d_grid[i];
            if (d_curr < d_plane_min || d_curr > d_plane_max) {  // If the current disparity is out of
                                                                 // the planes range
                u_warp = u - d_curr;
        ...
        for (d_curr = d_plane_min; d_curr <= d_plane_max; d_curr++) {
            ...
            if (u_warp < window_size || u_warp >= width - window_size)
                continue;
            updatePosteriorMinimum((__m128i *)(I2_line_addr + 16 * u_warp), d_curr, valid ? *(P + abs(d_curr - d_plane)) : 0, xmm1, xmm2, val,
                                   min_val, min_d);
        }
...
// for all triangles do
#pragma omp parallel for num_threads(3) default(none) private(i, plane_a, plane_b, plane_c, plane_d, c1, c2, c3) \
        ...
        if ((int32_t)(A_u) != (int32_t)(B_u)) {
            ...
            for (int32_t u = max((int32_t)A_u, 0); u < min((int32_t)B_u, width); u++) {
                ...
                if (!param.subsampling || u % 2 == 0) {
                    // Use linear lines, to get the bounds of where we need to
                    // check
                    ...
                    for (int32_t v = min(v_1, v_2); v < max(v_1, v_2); v++)
                        ...
                        if (!param.subsampling || v % 2 == 0) {
                            findMatch(u, v, plane_a, plane_b, plane_c, disparity_grid, grid_dims, I1_desc, I2_desc, P, plane_radius, valid,
                                      right_image, D);
                        }
                }
            }
                ...
                while (seg_list_curr < seg_list_count) {
                    ...
                    for (int32_t i = 0; i < 4; i++) {
                        ...
                        if (u_neighbor[i] >= 0 && v_neighbor[i] >= 0 && u_neighbor[i] < D_width && v_neighbor[i] < D_height) {
                            ...
                            // check if neighbor has not been added yet and if
                            // it is valid
                            if (*(D_done + addr_neighbor) == 0 && *(D + addr_neighbor) >= 0) {
                                ...
                                if (fabs(*(D + addr_curr) - *(D + addr_neighbor)) <= param.speckle_sim_threshold) {
                                    ...
                                    *(seg_list_v + seg_list_count) = v_neighbor[i];
                                    ...
                                    // (otherwise a pixel may be added 2 times
                                    // to the list, as
                                    //  neighbor of one pixel and as neighbor of
                                    //  another pixel)
...
void Elas::gapInterpolation(float *D) {
    ...
    for (int32_t u = 0; u < D_width; u++) {
        ...
        // added extrapolation to top and bottom since bottom rows sometimes
        // stay unlabeled... DS 5/12/2014
    ...
    if (param.subsampling) {
        ...
        for (int32_t v = 3; v < D_height - 3; v++) {
            ...
            for (int32_t u = 7; u < D_width; u++) {
                ...
                // Note this isn't truely the center since we have 8 for the
                // vestor registers
