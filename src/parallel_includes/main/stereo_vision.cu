...
void publishPointCloud(const Mat &img_left_old, Mat &dmap_old) {
    ...
    cudaMemcpy(d_dmap, dmap.data, sizeof(uchar) * point_cloud_width * point_cloud_height,
               cudaMemcpyHostToDevice);  // Causes invalid argument error sometimes (only while using as a shared library)
...
double3 *generatePointCloud(uchar *left,
    ...
    grapher->setColorsArray((uchar4 *)left_img_OLD.ptr<unsigned char>(0));
    if (objectTracking) {
...
void imageLoop() {
    ...
    for (unsigned iFrame = 0; (iFrame < max_files) && !graphicsThreadExit; ++iFrame) {
        ...
        grapher->setColorsArray((uchar4 *)rgba.ptr<unsigned char>(0));
        if (objectTracking) {
            auto f = std::async(std::launch::async, processYOLO, YOLOL_Color);  // Asynchronous call to YOLO
            resize(right_img, right_img_OLD, out_img_size);
            imgCallback_video();
