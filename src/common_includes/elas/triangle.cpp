...
void poolrestart(struct memorypool *pool) {
    ...
    pool->nextitem = (int *)(alignptr + (unsigned long)pool->alignbytes - (alignptr % (unsigned long)pool->alignbytes));
...
void traversalinit(struct memorypool *pool) {
    ...
    pool->pathitem = (int *)(alignptr + (unsigned long)pool->alignbytes - (alignptr % (unsigned long)pool->alignbytes));
...
void dummyinit(struct mesh *m, struct behavior *b, int trianglebytes, int subsegbytes) {
    ...
    m->dummytri = (triangle *)(alignptr + (unsigned long)m->triangles.alignbytes - (alignptr % (unsigned long)m->triangles.alignbytes));
    ...
    if (b->usesegments) {
        ...
        m->dummysub = (subseg *)(alignptr + (unsigned long)m->subsegs.alignbytes - (alignptr % (unsigned long)m->subsegs.alignbytes));
...
vertex getvertex(struct mesh *m, struct behavior *b, int number) {
    ...
    foundvertex = (char *)(alignptr + (unsigned long)m->vertices.alignbytes - (alignptr % (unsigned long)m->vertices.alignbytes));
...
float incircle(struct mesh *m, struct behavior *b, vertex pa, vertex pb, vertex pc, vertex pd) {
    ...
    permanent =
        (Absolute(bdxcdy) + Absolute(cdxbdy)) * alift + (Absolute(cdxady) + Absolute(adxcdy)) * blift + (Absolute(adxbdy) + Absolute(bdxady)) * clift;
...
float orient3d(struct mesh *m,
    ...
    permanent = (Absolute(bdxcdy) + Absolute(cdxbdy)) * Absolute(adheight) + (Absolute(cdxady) + Absolute(adxcdy)) * Absolute(bdheight) +
...
enum locateresult locate(struct mesh *m, struct behavior *b, vertex searchpoint, struct otri *searchtri) {
    ...
    while (totalsamplesleft > 0) {
        ...
        firsttri = (char *)(alignptr + (unsigned long)m->triangles.alignbytes - (alignptr % (unsigned long)m->triangles.alignbytes));
...
void triangulate(char *triswitches, struct triangulateio *in, struct triangulateio *out, struct triangulateio *vorout) {
    ...
    if (b.voronoi) {
        writevoronoi(&m, &b, &vorout->pointlist, &vorout->pointattributelist, &vorout->pointmarkerlist, &vorout->edgelist, &vorout->edgemarkerlist,
                     &vorout->normlist);
    }
    if (b.neighbors) {
