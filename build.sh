...
if [ $1 == "3" ]
	then
		...
		nvcc -o bin/stereo_vision src/stereo_vision.cu src/elas_openmp/*.cpp  src/yolo/*.cpp src/graphing/*.cu -lpopt -lglut -lGLU -lGL -std=c++11 -Xcompiler="-pthread -fopenmp" -lm `pkg-config --cflags --libs opencv` -w && echo "Compiled Successfully!! Run the program using ./bin/stereo_vision -k path_to_kitti -v 1 -p 0 -f 1"
		exit 0
fi
