...
if [ $1 == "0" ]
	then
		...
		g++ -o bin/stereo_vision src/stereo_vision.cpp src/elas/*.cpp src/graphing/*.cpp src/nlohmann/*.hpp -lpopt -lglut -lGLU -lGL -std=c++11 -pthread `pkg-config --cflags --libs opencv` -w	
		exit 0
fi
