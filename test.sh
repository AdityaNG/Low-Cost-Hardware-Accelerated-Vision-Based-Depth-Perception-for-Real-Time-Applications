...
do
    ...
    do
        ...
        echo "CPU" >> tmp.txt
        OMP_NUM_THREADS=1 ./build/bin/stereo_vision_serial -k $(pwd)/kitti_mini/ -v=1 -s=$SUBSAMPLING_MODE -p=0 -f=$SCALE_FACTOR | grep AVG_FPS= >> tmp.txt
        echo "OMP" >> tmp.txt
        OMP_NUM_THREADS=12 ./build/bin/stereo_vision_serial -k $(pwd)/kitti_mini/ -v=1 -s=$SUBSAMPLING_MODE -p=0 -f=$SCALE_FACTOR | grep AVG_FPS= >> tmp.txt
        echo "CUDA" >> tmp.txt
